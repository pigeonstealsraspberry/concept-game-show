...
export function PlayerAvatar() {
  ...
  useFrame((state) => {
    ...
    groupRef.current.position.set(PLAYER_DESK_X, -0.1, 0.1)
    groupRef.current.rotation.y = Math.atan2(camZ - 0.1, camX - PLAYER_DESK_X) - Math.PI

    const isAnswering = overlayPhase === 'answer' && ['round1_name', 'round1_company', 'round2_email', 'round3_category', 'round4_wheel'].includes(step)
    if (bodyRef.current) {
      bodyRef.current.position.y = isAnswering ? 0.55 + Math.sin(t * 0.8) * 0.02 : 0.55
    }
    if (armLRef.current && armRRef.current) {
      armLRef.current.rotation.x = 0.35
